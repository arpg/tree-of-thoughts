    ...
    def solve(
        ...
        def dfs(state, step):
            ...
            filtered_thoughts = [
                thought
                for thought in thoughts
                if evaluated_thoughts[thought] >= pruning_threshold
            ]
    ...
    def solve(
        ...
        while not open_set.empty() and len(visited_states) < max_steps:
            ...
            for thought in thoughts:
                ...
                tentative_g_score = g_scores[current_state] + self.cost(
                    current_state, next_state
                )
                if (
                    next_state not in g_scores
                    or tentative_g_score < g_scores[next_state]
                ):
                    came_from[next_state] = current_state
                    g_scores[next_state] = tentative_g_score
                    f_scores[next_state] = tentative_g_score + self.heuristic(
                        next_state
                    )
...
class MonteCarloTreeofThoughts(TreeofThoughts):
    ...
    def solve(
        self, initial_prompt, num_thoughts, max_steps, max_states, exploration_constant
    ):
        ...
        best_state = max(
            self.tree["nodes"][initial_prompt]["children"],
            key=lambda child: self.tree["nodes"][child]["value"],
        )
        return best_state
    ...
    def select(self, state):
        while self.tree["nodes"][state]["children"]:
            state = max(
                self.tree["nodes"][state]["children"],
                key=lambda child: self.ucb1(child),
            )
        return state
    ...
    def expand(self, state, num_thoughts, initial_prompt):
        if state not in self.tree["nodes"]:
            ...
            for thought in thoughts:
                next_state = (
                    (state, thought) if isinstance(state, str) else (*state, thought)
                )
                self.tree["nodes"][state]["children"].append(next_state)
                self.tree["nodes"][next_state] = {
                    "visits": 0,
                    "value": 0,
                    "children": [],
                }
